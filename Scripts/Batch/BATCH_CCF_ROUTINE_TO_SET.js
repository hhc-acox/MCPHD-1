/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_CCF_ROUTINE_TO_SET.js  Trigger: Batch
| Client:
|
| Version 1.0 - Initial
|
...
var setCode = 'WQ_CCF_ROUTINE_INSPECTIONS'
var setType = 'CCF Routines'
...
function mainProcess() {
    ...
    cm.getCapType().setSubType("Childcare");
    ...
    if (getCaps.getSuccess()) {
        ...
        for (x in gc) {
            ...
            if (matches(String(capStatus).toUpperCase(), "EXPIRED", "INACTIVE", "REVOKED", "CLOSED")) {
                capFilterStatus++;
                continue;
            }
