...
if (wfTask == 'Initial Processing' && matches(wfStatus,'Complete Notice of Violation','Complete Emergency NOV') && AInfo['Reinspection Date (in 14 days if blank)'] != null && AInfo['Reinspection Date (in 14 days if blank)'] != '') {
	scheduleInspectDate('Reinspection',AInfo['Reinspection Date (in 14 days if blank)'],AInfo['Assigned To']);
	}

if (wfTask == 'Initial Processing' && matches(wfStatus,'Complete Notice of Violation','Complete Emergency NOV') && (AInfo['Reinspection Date (in 14 days if blank)'] == null || AInfo['Reinspection Date (in 14 days if blank)'] == '')) {
	scheduleInspectDate('Reinspection',nextWorkDay(dateAdd(null,13)),AInfo['Assigned To']), editTaskSpecific('Initial Processing','Reinspection Date (in 14 days if blank)',nextWorkDay(dateAdd(null,13)));
	}
...
if (wfTask == 'Additional Processing' && AInfo['Owner Occupied'] == 'Yes') {
	cleanType = 'Occupied';
	}

if (wfTask == 'Additional Processing' && AInfo['Owner Occupied'] == 'No') {
...
//end replaced branch: CMN;HHC!TRA!~!~AddTicketFee
	...
	assignTask('Ticket',AInfo['Assigned To']);
	}
