...
if (wfTask == 'Initial Processing' && (wfStatus == 'Complete Notice of Violation') && AInfo['Reinspection Date (in 21 days if blank)'] != null && AInfo['Reinspection Date (in 21 days if blank)'] != '') {
	scheduleInspectDate('Reinspection',AInfo['Reinspection Date (in 21 days if blank)'],AInfo['GENERAL.Assigned To']);
	}

if (wfTask == 'Initial Processing' && (wfStatus == 'Complete Notice of Violation') && (AInfo['Reinspection Date (in 21 days if blank)'] == null ||AInfo['Reinspection Date (in 21 days if blank)'] == '')) {
	scheduleInspectDate('Reinspection',nextWorkDay(dateAdd(null,20)),AInfo['GENERAL.Assigned To']);
