/*----------------------------------------------------------------------------------------------------------------------/
| Program:  ASSIGN_WATERSHED_INSPECTIONS.js
| Trigger:  SET
| Client:   
| Version:  1.0
| Author:   Jake Cox
...
/------------------------------------------------------------------------------------------------------*/
//if (paramsOK){
logMessage("START","Start of Assign Watershed Inspections Set Script.");
...
function processRecords(Collection)
{
	...
	var standardChoice = getStandardChoiceValues('WQ_Watershed_Assign_Routes');
	...
	for (i in thisCapCollection)
	{
		...
		for (i in standardChoice) {
			//aa.print(standardChoice[i].key);
			if(asiValue == standardChoice[i].key && standardChoice[i].value != null) {
				scheduleInspectDate('Routine',tomorrow,standardChoice[i].value);
				aa.print("Assigning Inspection for: " + altID + " --- To: " + standardChoice[i].value);
				capCount++;
			}
		}
