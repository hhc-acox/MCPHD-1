/*------------------------------------------------------------------------------------------------------/
| Set Script - Vending Machine Renewal
| 
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
try {
    ...
    for (var i=0; i < SetMemberArray.length; i++) {
    	...
    	numMachines = getAppSpecific("Number of Machines", capId)
    	if (numMachines == "1 through 50") {
    		feeSeq = addFee("FS0010", "FS_GENERAL", "FINAL", 1, "N");
    		invoiceOneNow(feeSeq, "FINAL", capId);
    	}
    	if (numMachines == "51 through 300") {
    		feeSeq = addFee("FS0011", "FS_GENERAL", "FINAL", 1, "N");
    		invoiceOneNow(feeSeq, "FINAL", capId);
    	}
    	if (numMachines == "Over 300") {
    		feeSeq = addFee("FS0012", "FS_GENERAL", "FINAL", 1, "N");
    		invoiceOneNow(feeSeq, "FINAL", capId);
    	}
